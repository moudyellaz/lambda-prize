...
- [ ] Bidders can submit bids as shielded transfers to the auction contract without the bid amount being visible to other participants or on-chain observers.
- [ ] The auction contract determines the winner entirely on-chain, without a trusted auctioneer or off-chain reveal phase.
...
- A LEZ auction contract (Rust) implementing the full lifecycle: auction creation, shielded bid submission, winner determination, and refunds.
...
- Public repository with LEZ contract code and client-side tooling under MIT or Apache-2.0.
